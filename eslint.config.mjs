  ...
  //   → main-frontend-impl / main-ipc → main-entry
  {
    ...
    settings: {
      ...
      'boundaries/elements': [
        ...
        // main-types：cross-process 类型合同层
        // - src/main/types/*：纯类型
        // - src/main/frontend/core/types.ts：ChatEvent 等事件类型（preload / renderer 可消费）
        { type: 'main-types', pattern: 'src/main/types' },
        { type: 'main-types', pattern: 'src/main/frontend/core/types.ts', mode: 'file' },
        ...
        // main-frontend-core：frontend/core 里 types.ts 以外的运行时编排（Gateway / Registry / OperationContext）
        // 需要显式在更具体的 pattern（main-types 的 types.ts 文件模式）之后声明
      ...
      'boundaries/dependencies': [
        'error',
        {
          ...
          rules: [
            ...
            // main-service-module：services/ 下的独立子目录模块（bundler / browser / widget / stt / tts）
