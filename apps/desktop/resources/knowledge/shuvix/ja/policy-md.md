...
- `inDir(path, dirs)` —— `dirs` は文字列かリスト。`path` がそのいずれかの中にあれば真（パスセグメント
  境界で判定：`/foo` は `/foobar` に一致しない）。空や文字列でない項目は決して一致しない。デスクトップでは
  両側とも**実際に行き着く場所**で比較する：`path` と各ディレクトリを先に解決（シンボリックリンク、`..`、
  ディスク上の大文字小文字）してから比べる —— ワークスペース内の `~/.ssh/id_rsa` を指すリンクは `~/.ssh`
  の中とみなされ、`~/.ssh` 自体が dotfiles リポジトリへのリンクでも一致する。相対パスのディレクトリは
  書かれたまま比較する。
