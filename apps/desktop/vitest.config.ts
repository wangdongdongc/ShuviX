...
export default defineConfig({
  resolve: {
    alias: {
      ...
      // 内置 agent / 安全策略 md 的**构建期内联**变体（只给单测用；主进程读随包目录）。
  ...
  test: {
    // 测试 main 进程、CLI 及 Node.js 共享代码；外加共享包 chat-protocol / agent-runtime / chat-ui
    // 的单测，以及 Chrome 扩展里的纯逻辑单测（扩展不自配 vitest —— 桥的两端对照要能一条命令
    // 一起跑，分成两套 runner 只会让其中一端悄悄烂掉）
    include: [
      'src/main/**/*.test.ts',
      'src/cli/**/*.test.ts',
