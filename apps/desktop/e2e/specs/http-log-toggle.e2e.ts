...
beforeAll(async () => {
  app = await launchApp()
  settings = await app.openSettings('httpLogs')
...
describe('LLM 日志记录开关', () => {
  ...
  it('关闭时列表空态给出「记录已关闭」而非「暂无日志」', async () => {
    // 开关状态是异步读出来的（读到之前空态按中性的「暂无日志」渲染），轮询到落定
    const text = await until(async () => {
      const current = await pane.emptyText()
      return /关闭|off|オフ/i.test(current) ? current : ''
    }, 'disabled hint in empty list')
