 ...
 * 桌面 SecurityContext）。read/write/edit 三个工具共用同一份装配；read 额外注入内容解码器。
 *
 * 配置（workingDirectory/allowList）按工具执行时机动态解析（resolveProjectConfig），
 * 故 resolvePath / security 内部都走惰性 thunk，保证拿到最新值（对齐原 ReadTool/WriteTool 行为）。
 */
