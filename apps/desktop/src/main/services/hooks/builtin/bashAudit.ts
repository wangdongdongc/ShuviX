/**
 * 内置 hook：危险命令审计（PreToolUse on bash）
 *
 * 命中以下任一模式立即拒绝（同时写 logger.warn）：
 *
 * **POSIX**：
 * - `rm -rf /`             根目录递归删除
 * - `:(){ :|:& };:`        bash fork 炸弹
 ...
 * 复杂的 sandbox / chroot 不在此 hook 范围。
 */

import { createLogger } from '../../../logger'
import type { HookHandler, HookInput, HookOutput } from '../types'

const log = createLogger('Builtin:bashAudit')
...
export const bashAuditHandler: HookHandler = (input: HookInput): HookOutput | void => {
  ...
  if (hit) {
    log.warn(`bash-audit 拦截: ${hit} session=${input.session_id} command="${cmd.slice(0, 200)}"`)
    return {
      hookSpecificOutput: {
