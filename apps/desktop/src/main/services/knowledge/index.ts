 ...
 *
 * **两个根、一个绑定实体一个 bundle**：`~/.shuvix/knowledge/` 是用户的库（本期只占名字），
 * `~/.shuvix/knowledge-shuvix/` 是 ShuviX 维护的，本期只有 `projects/<slug>/`。核心逻辑在
 * @shuvix/agent-runtime 的 knowledge/，这里只做宿主该做的：两个根、bundle 的建立与定位、
 * 扫描缓存、按 bundle 的 index/log 投影与 git 提交、检索索引、写入者 actor。
 *
...
export { sessionBundle, type SessionBundleTarget } from './sessionBundle'
export { scanBundle, scanAllBundles, listBundles, invalidateKnowledgeScan } from './scan'
