...
/**
 * 内置 hook 目录 —— **内置 hook 的唯一事实源**。随包发布（`Resources/builtin-hooks/`，
 * 见 electron-builder.yml 的 extraResources），开发期指向仓库里
 * `packages/agent-runtime/src/hook/builtinHooks/md`。
 *
 * 运行时按当前语言现读这里的文件（hookService → buildBuiltinHooks 的 readMd），侧栏点开
 * 一份内置 hook 时打开的也是同一份文件的只读笔记本。与 getBuiltinAgentsDir 同策：
 * 没有 app（半桩的单测）按未打包算。
 */
export function getBuiltinHooksDir(): string {
  return app?.isPackaged
    ? join(process.resourcesPath, 'builtin-hooks')
    : resolve(__dirname, '../../../../packages/agent-runtime/src/hook/builtinHooks/md')
}
