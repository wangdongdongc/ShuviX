 ...
 * `database` —— 已保存的数据库连接；`chrome` —— Chrome 扩展的连接状态与安装。前两台是按会话勾选的
 * 内置 MCP 能力服务器，`chrome` 只由 Chrome 侧边栏会话使用，设置都跟着它们放在这里。
 */
  ...
  return (
    <McpClientPanel
      ...
      renderServerExtra={(server) => {
        ...
        if (server.name === 'chrome') return <ChromeExtensionSettings />
        return null
      }}
    />
  )
