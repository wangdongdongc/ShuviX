/**
 * 监视器（桌面设置页）—— 各类运行记录的统一去处：LLM 请求 / MCP 调用。
 *
...
export type MonitorSubTab = 'httpLogs' | 'mcpCalls'

/** 合法子 tab（供 hash 路由 `#settings/monitor/<sub>` 校验） */
export const MONITOR_SUB_TABS = new Set<string>(['httpLogs', 'mcpCalls'])
  ...
  return (
    <div className="flex flex-col h-full min-h-0">
      ...
      <div className="flex-1 min-h-0 flex flex-col">
        {subTab === 'httpLogs' ? <HttpLogSettings /> : <McpServerLogsPanel />}
      </div>
    </div>
  )
}
