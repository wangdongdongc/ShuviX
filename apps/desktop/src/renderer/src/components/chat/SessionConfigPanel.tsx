...
export function SessionConfigPanel({ sessionId }: { sessionId: string }): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    const loadShareUrls = async (): Promise<void> => {
      const status = await getChatApi().webui.serverStatus()
      if (cancelled) return
      const urls =
    ...
    const loadBots = async (): Promise<void> => {
      const bots = await getChatApi().telegram.listBots()
      if (cancelled) return
      setTelegramBots(
        bots.map((b) => ({
          id: b.id,
  ...
  const handleSetShareMode = async (mode: ShareMode | null): Promise<void> => {
    await getChatApi().webui.setShared({
      sessionId,
      shared: mode !== null,
      mode: mode ?? undefined
    })
  ...
  const handleSelectTelegramBot = async (botId: string | null): Promise<void> => {
    ...
    if (botId) {
      await getChatApi().telegram.bindSession({ botId, sessionId })
    }
