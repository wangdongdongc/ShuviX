...
export function buildCmTheme(opts?: { maxHeight?: string }): Extension {
  const base = EditorView.theme(
    {
      // maxHeight（内嵌模式）：编辑器随内容长高、到顶后 .cm-scroller 内部滚动（保住视口虚拟化）；
      // 否则撑满父容器（全屏预览模式）
      '&': {
        backgroundColor: 'var(--theme-bg-primary)',
        color: 'var(--theme-text-primary)',
        ...(opts?.maxHeight ? { maxHeight: opts.maxHeight } : { height: '100%' })
      },
      '.cm-scroller': {
        fontFamily:
