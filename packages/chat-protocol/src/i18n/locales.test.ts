...
describe('i18n 语言包', () => {
  ...
  /**
   * EO-52：「用别的应用打开这个链接？」询问框的六句文案（面板、主窗口、设置、悬浮聊天、widget
   * 共用这一份）。目标地址与发起者是不可信字符串，由 services/externalOpen 逐行拼进 detail ——
   * 这几句里一旦出现 `{{x}}`，就是又有人把地址交给了 i18next 插值（插值会连地址里写的 `{{…}}`
   * 一起换掉，框里显示的就不是要打开的那个地址了）。旧的 `browser.external.*` 是它上一个家，
   * 留着会有人继续往那儿加文案。open 与 cancel 同字，两个按钮就分不出来。
   */
  it('EO-52 externalOpen.* 六句三语都非空、都不含 {{x}} 插值，open ≠ cancel；旧的 browser.external.* 三语都已下线', () => {
    const EXTERNAL_KEYS = ['title', 'fromPanel', 'fromWidget', 'hint', 'open', 'cancel'].map(
      (name) => `externalOpen.${name}`
    )
    for (const [lang, bundle] of Object.entries({ en, zh, ja })) {
      ...
      expect(leaf(bundle, 'externalOpen.open'), lang).not.toBe(leaf(bundle, 'externalOpen.cancel'))
      const oldKeys = flatten(bundle).filter((k) => /^browser\.external(\.|$)/.test(k))
      expect(oldKeys, lang).toEqual([])
    }
