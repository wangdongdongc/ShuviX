/**
 * applyWrite / applyEdit 的审批接线单测 —— 内存 FileSystemPort + 假 FileGuards（写锁是真的串行链）。
 *
 * 核心不变量：审批卡片看到的 diff 与 tool result details.diff 是同一份（一次算成、一次封顶）；
 * 审批在 port.writeFile 之前、在文件锁内发生；审批期间文件被外部改动一律作废重来。
 */
...
interface Harness {
  port: FileSystemPort
  guards: FileGuards
  /** 有序事件流：'approve:<path>' / 'write:<path>' —— 用于断言审批发生在写入之前 */
  events: string[]
...
describe('applyEdit/applyWrite — preview diff equals executed diff', () => {
  it('CONS-1: edit hands the approval hook the exact string it later returns in details.diff', async () => {
    const h = makeHarness()
    h.seedFile(P, 'alpha\nbeta\ngamma\n')
    h.seedRead(P)
    const approve = spyApprove(h)
    ...
    expect(approve.calls[0].diff).toBe(res.details.diff)
  ...
  it('CONS-2: write of a new file previews an all-additions diff with isNewFile on both sides', async () => {
    ...
    expect(approve.calls[0].isNewFile).toBe(true)
  ...
  it('CONS-3: write over an existing file previews a diff against the old content, isNewFile false', async () => {
    ...
    expect(approve.calls[0].isNewFile).toBe(false)
  ...
  it('CONS-4: a >1200-line change is truncated once, and both sides get that same truncated string', async () => {
    ...
    const res = await applyWrite(h.port, h.guards, P, { path: P, content }, approve.hook)

    const diff = res.details.diff
    expect(approve.calls[0].diff).toBe(diff)
...
describe('applyEdit/applyWrite — approval ordering and re-validation', () => {
  it('CONS-8: approve runs before port.writeFile; throwing from it writes nothing', async () => {
    const h = makeHarness()
    h.seedFile(P, 'alpha\nbeta\n')
    h.seedRead(P)

    const ok = spyApprove(h)
    await applyEdit(h.port, h.guards, P, { path: P, oldText: 'beta', newText: 'B1' }, ok.hook)
    expect(h.events).toEqual([`approve:${P}`, `write:${P}`])

    const denied = spyApprove(h, () => {
  ...
  it('CONS-16: recordRead is not called when approve throws', async () => {
    const h = makeHarness()
    h.seedFile(P, 'alpha\n')
    h.seedRead(P)
    const denied = spyApprove(h, () => {
  ...
  it('CONS-9 (edit): an external change during approval aborts the edit', async () => {
    const h = makeHarness()
    h.seedFile(P, 'alpha\nbeta\n')
    h.seedRead(P)
    const approve = spyApprove(h, () => {
    ...
    await expect(
      applyEdit(h.port, h.guards, P, { path: P, oldText: 'beta', newText: 'BETA' }, approve.hook)
  ...
  it('CONS-9 (write, never read): a file created during approval aborts the write', async () => {
    const h = makeHarness()
    const approve = spyApprove(h, () => {
    ...
    ).rejects.toThrow(`${P} changed while waiting for approval; re-read it and try again`)
  ...
  it('CONS-9 (write, never read): an existing file changed during approval aborts the write', async () => {
    const h = makeHarness()
  ...
  it('CONS-9: the post-approval re-check only runs when an approve hook was supplied', async () => {
    const h = makeHarness()
    h.seedFile(P, 'original\n')

    // write 无 approve：没读过 → 前置检查跳过；无 approve → 事后复检也跳过
    await applyWrite(h.port, h.guards, P, { path: P, content: 'ours\n' })
    expect(h.assertCalls).toEqual([])

    // edit 无 approve：只有开头那一次前置检查
    h.seedRead(P)
    await applyEdit(h.port, h.guards, P, { path: P, oldText: 'ours', newText: 'theirs' })
    expect(h.assertCalls).toEqual([P])
  })

  it('CONS-10: concurrent edits serialize — the second approval sees the first write', async () => {
    const h = makeHarness()
    h.seedFile(P, 'line1\nline2\nline3\n')
    h.seedRead(P)
    ...
    const first = spyApprove(h, () => firstGate)
    const second = spyApprove(h)
    ...
    expect(h.events).toEqual([`approve:${P}`, `write:${P}`, `approve:${P}`, `write:${P}`])
...
describe('applyEdit/applyWrite — failures before approval', () => {
  it('REG-1: a non-matching oldText fails before approval', async () => {
    const h = makeHarness()
    h.seedFile(P, 'alpha\n')
    h.seedRead(P)
    const approve = spyApprove(h)
    ...
    await expect(
      applyEdit(h.port, h.guards, P, { path: P, oldText: 'nope', newText: 'x' }, approve.hook)
    ).rejects.toThrow(/No match found/)
    expect(approve.calls).toEqual([])
    expect(h.writes).toEqual([])
  })

  it('REG-1: "No change produced" fails before approval', async () => {
    const h = makeHarness()
    h.seedFile(P, 'alpha\n')
    h.seedRead(P)
    const approve = spyApprove(h)
    ...
    await expect(
      applyEdit(h.port, h.guards, P, { path: P, oldText: 'alpha', newText: 'alpha' }, approve.hook)
    ).rejects.toThrow(`No change produced: ${P}`)
    expect(approve.calls).toEqual([])
    expect(h.writes).toEqual([])
  })

  it('REG-2: editing a missing file reports File not found before approval', async () => {
    const h = makeHarness()
    const approve = spyApprove(h)

    await expect(
      applyEdit(h.port, h.guards, P, { path: P, oldText: 'a', newText: 'b' }, approve.hook)
