 ...
 *  1. 兜住脏能力数据。litellm 会把窗口值原样填进 max_output_tokens（实测 xai/grok-4.6：
 *     max_input = max_output = 500000；随包目录 1976 条 chat 模型里 751 条是这个形状）。
 *     不封顶的话 reserve 直接被顶过窗口、阈值变负，**每一轮都触发压缩**。源头已在
 *     modelResolver 归一（输出上限 ≥ 窗口视为未知），这里是不依赖识别脏不脏的第二道防线。
 *  2. 正常的大输出模型若按理论上限预留，会白白让出可观的窗口（GPT-5 272k/128k 下
 *     reserve 占窗口 57% → 22%，Claude 200k/64k 下 42% → 26%）。代价要如实记：
 *     pi 在每个 API 的请求构造里都把 max_tokens 夹到「window − 上下文 − 4096」，所以
 *     「输入 + 申请输出 > window」的整请求 400 本就不会发生；阈值处留给单次输出的余量是
 *     cap + 10%×window − 4k（200k 窗口约 48k），只有单次输出超过它的大文件写入才会被
 *     截断（stopReason='length'）。纯文本截断内容可用；带工具调用的消息 pi 会把**整批**
 *     调用判错并让模型重发，重发时预算更紧（失败结果已进上下文）。这是拿罕见场景的
 *     一次重做，换全类脏数据免疫和窗口利用率。
 */
