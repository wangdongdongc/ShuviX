/**
 * RuntimeAgent —— 宿主无关的 Agent 运行时引擎（原 RuntimeSession）。
 *
 * 封装单个 agent 的 pi-agent-core Agent 生命周期、事件转发、流式落库与用户输入挂起。
 * 所有 agent 都是本类实例、地位对等：会话根 agent（persistence=数据库）与派生任务
 * agent（persistence=内存 ephemeral，不产生会话记录）仅差在注入的持久化实现；
 * 父子关系经 parentAgentId / depth 表达，与「是否有 session」正交。
 *
...
export interface RuntimeAgentDeps {
  /** 本 agent 的事件频道 id：会话根 agent = sessionId；派生 agent = 其 agentId */
  sessionId: string
  /** 派生来源 agent 的 id（会话根 agent 省略） */
  parentAgentId?: string
  /** 派生层级（会话根 agent = 0，缺省 0） */
  depth?: number
...
export class RuntimeAgent {
  ...
  /** 派生来源 agent 的 id（会话根 agent 为 undefined） */
  ...
  constructor(deps: RuntimeAgentDeps) {
    this.sessionId = deps.sessionId
    this.parentAgentId = deps.parentAgentId
  ...
  /**
   * 运行时信息快照 —— 直接读内存中的 agent.state，与实际请求 LLM 使用的
   * systemPrompt / 工具集 / 模型零漂移（供前端「Agent 信息」弹窗只读展示，各端共用）。
   */
