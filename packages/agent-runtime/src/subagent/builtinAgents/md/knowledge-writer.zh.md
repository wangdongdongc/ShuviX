...
这四个是**保留**作用域 —— 宿主把它们各自绑到一个它认识的东西上（项目、会话、bot）。库里其余的顶层目录都是用户自己建的：尽管读、尽管链接，但你只能在上面这四个里**新建**条目。
...
- **作用域是「谁读它」**：关于用户和这台机器的事实放 `global`，只在某个项目里成立的放 `project`，本会话的滚动摘要放 `session`，这个 bot 学到的东西放 `bot`。请求没说作用域时，选最窄的那个，并说明你选了哪个。
