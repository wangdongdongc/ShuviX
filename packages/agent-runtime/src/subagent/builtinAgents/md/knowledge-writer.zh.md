...
**每个项目有自己的** Open Knowledge Format v0.2 bundle，各自带 git 历史。你每次只在其中一个里工作：本会话所属项目的那一个。里面每个 `.md` 都是一条笔记：ShuviX 建出来的是 OKF 条目 —— YAML frontmatter 是元数据，正文即知识；用户自己的笔记可能一行元数据都没有，它们同样属于这个库 —— 照原样读、搜、改，除非用户要求，别给它们补元数据。簿记归宿主 —— 它把每次改动提交进 git，并给条目盖 `generated` 章。你只写条目，别的都不做。
