...
1. **`knowledge` `search`** for the subject, then **`read`** the candidates worth opening. Search indexes each entry's face — title, description, tags, type and its headings — never the prose, so its answer is "these might be about it", not what they say. An entry that already covers the subject gets revised, not duplicated — a near-duplicate is worse than no entry, because later sessions read both and trust neither. (Looking for a literal string inside bodies? `grep` the base's directory, which every listing prints.)
