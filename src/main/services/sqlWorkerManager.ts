 ...
 * 管理每个 session 的 PGLite worker_threads 实例
 */
...
class SqlWorkerManager {
  ...
  private async createWorker(
    ...
    log.info(`Creating SQL worker for session ${sessionId}: ${workerPath}`)
    ...
    return new Promise<void>((resolveInit, rejectInit) => {
      ...
      worker.on('message', (msg: WorkerResponse) => {
        if (msg.type === 'ready') {
          ...
          entry.ready = true
          log.info(`SQL worker ready for session ${sessionId}`)
          onReady?.()
          resolveInit()
          return
        }
      ...
      worker.on('error', (err) => {
        clearTimeout(initTimeout)
        log.error(`SQL worker error (session ${sessionId}):`, err)
      ...
      worker.on('exit', (code) => {
        log.info(`SQL worker exited (session ${sessionId}, code ${code})`)
  ...
  async execute(
    ...
    return new Promise<WorkerResponse>((resolve, reject) => {
      const timer = setTimeout(() => {
        ...
        log.warn(`SQL execution timed out (${timeoutMs}ms), terminating worker`)
        this.terminate(sessionId)
  ...
  terminate(sessionId: string): void {
    ...
    log.info(`Terminating SQL worker for session ${sessionId}`)
    for (const [, req] of entry.pending) {
      clearTimeout(req.timer)
