 ...
 * - 目录 / manifest 写入
 * - DB 记录同步
 * - 把 widget 目录注入会话 allowList，便于跨会话读写
...
export interface InitWidgetParams {
  ...
  /** 调用来源会话 — 若提供，自动把 widget 目录加入该会话的 allowList（读写） */
  sessionId?: string
}
...
class WidgetService {
  ...
  async init(params: InitWidgetParams): Promise<InitWidgetResult> {
    ...
    if (params.sessionId) {
      this.grantSessionAccess(params.sessionId, dir)
    }
  ...
  async build(id: string, sessionId?: string): Promise<BuildWidgetResult> {
    this.validateId(id)
    ...
    if (sessionId) this.grantSessionAccess(sessionId, dir)
  ...
  /** 为指定会话注入对该 widget 目录的读写 allowList 条目（若已存在则不重复） */
  grantSessionAccess(sessionId: string, dir: string): void {
