...
/** 前导环境变量赋值形式:`NAME=...`(POSIX 约定,作用于紧随其后的命令) */
const ENV_ASSIGN_RE = /^[A-Za-z_][A-Za-z0-9_]*=/

/** 跳过开头的 `VAR=value` 环境变量赋值,返回真正的命令 tokens */
 ...
 * - 前导 `VAR=value` 环境变量赋值会被跳过(如 `GOOS=js go build ...` → `go build *`)
 */
export function toPattern(command: string): string {
  const raw = tokenize(command.trim())
  if (raw.length === 0) return command.trim()
  const tokens = stripLeadingEnv(raw)
  if (tokens.length === 0) return raw.join(' ')
...
export function isCommandAllowed(allowList: string[] | undefined, command: string): boolean {
  ...
  return units.every((unit) => {
    // 剥离前导 env 赋值后再与 allowList 条目比对
    const raw = tokenize(unit.trim())
    const stripped = stripLeadingEnv(raw)
    const normalized = (stripped.length > 0 ? stripped : raw).join(' ')
    return allowList.some((entry) => matchesEntry(entry, normalized))
  })
}
