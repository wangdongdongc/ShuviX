...
/**
 * Dev plugin —— 贡献 3 个斜杠命令，分别对应 `dev` 工具的三种 kind：
 ...
 *
 * 所有运行时逻辑（打包、dev server、库管理）由 dev 工具 + widgetServer / projectManager 完成。
 */
const devPlugin: ShuviXPlugin = {
  id: 'dev',
  name: 'Dev Studio',
  version: '1.0.0',

  activate(_ctx: PluginContext): PluginContribution {
    return {
      commands: [
        {
          commandId: 'widget',
          ...
          requiredTools: ['dev', 'read', 'write', 'edit', 'preview']
        },
        {
...
5. **Preview.** Call the \`preview\` tool with \`action: "open"\` and the url. Subsequent rebuilds live-reload via SSE.
...
4. \`dev\` \`action: "build"\` \`kind: "widget"\` with the id — preview auto-refreshes.
...
4. Call the \`preview\` tool with \`action: "open"\` and the URL.
5. If the build fails, fix the errors and call \`dev\` build again. Subsequent builds live-reload via SSE — no need to reopen the preview.
...
4. Call the \`preview\` tool \`action: "open"\` with the URL. Further builds live-reload via SSE.
